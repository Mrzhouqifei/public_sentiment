    ...
    public void mouseClicked(PreviewMouseEvent event, PreviewProperties properties, Workspace workspace) {
        ...
        for (Node node : Lookup.getDefault().lookup(GraphController.class).getGraphModel(workspace).getGraph().getNodes()) {
            if (clickingInNode(node, event)) {
                ...
                System.err.println("Node " + node.getId()+ " clicked!");//System.out is ignored in Netbeans platform applications!!
                
                System.err.println(node.x()+ "," + node.y());
                JOptionPane.showMessageDialog(null, "Node " + node.getId()+ " clicked!");
